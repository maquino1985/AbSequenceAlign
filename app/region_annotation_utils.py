...
from app.numbering_conversion_utils import convert_numbering
...
SCHEME_MAP = {
    ("imgt", "H"): IMGT_HEAVY,
    ("imgt", "L"): IMGT_HEAVY,  # IMGT light chain boundaries are similar, can be replaced with IMGT_LIGHT if needed
    ("kabat", "H"): KABAT_HEAVY,
    ("kabat", "L"): KABAT_LIGHT,
...
def find_kabat_cdrs(numbering: List[Tuple[Tuple[int, str], str]], chain_type: str) -> Dict[str, Tuple[int, int]]:
    """
...
def annotate_all_schemes(
    ...
    out = {}
    for sch in schemes:
        converted = convert_numbering(numbering, from_scheme, sch, chain_type=chain_type)

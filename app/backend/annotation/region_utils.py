...
class RegionIndexHelper:
    ...
    def build_pos_to_idx(numbering: List[Tuple[Any, Any]]) -> Dict[Any, int]:
        ...
        """
        return {num[0]: idx for idx, num in enumerate(numbering) if isinstance(num, tuple) and len(num) > 1}

    @staticmethod
    def find_region_indices(pos_to_idx: Dict[Any, int], start: Any, stop: Any) -> Tuple[int, int]:
    ...
    @staticmethod
    def extract_region_sequence(numbering: List[Tuple[Any, Any]], start_idx: int, stop_idx: int) -> str:

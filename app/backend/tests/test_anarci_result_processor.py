...
from .conftest import SCFV_SEQ, IGHG1_SEQ, KIH_SEQ, TCR_SEQ
from backend.logger import logger

@pytest.mark.parametrize("seq_dict_name,expected_v_domains,expected_c_domains", [
...
def test_anarci_result_processor_domains(seq_dict_name, expected_v_domains, expected_c_domains, 
                                       SCFV_SEQ, IGHG1_SEQ, KIH_SEQ, TCR_SEQ):
    seq_dicts = {
        "scfv": SCFV_SEQ,
        "ighg1": IGHG1_SEQ,
        "kih": KIH_SEQ,
    ...
    assert len(v_domains) == expected_v_domains, f"Expected {expected_v_domains} variable domains, got {len(v_domains)}"
    assert len(c_domains) == expected_c_domains, f"Expected {expected_c_domains} constant domains, got {len(c_domains)}"
    ...
    for chain in result_obj.chains:
        ...
        for domain in chain.domains:
            ...
            if domain.domain_type == 'C' and domain.constant_region_info:
                ...
                logger.info(f"    Position: {domain.constant_region_info['start']}-{domain.constant_region_info['end']}")
...
def test_scfv_chain_preservation(SCFV_SEQ):
    ...
    chain = result.chains[0]
    assert len(chain.domains) == 2, "scFv should have 2 variable domains"
    
    # Verify domain order and linker detection
    domains = chain.domains
    assert domains[0].domain_type == 'V'
    assert domains[1].domain_type == 'V'
    
    # Check for linker information
    assert 'preceding_linker' in domains[1].alignment_details
    linker = domains[1].alignment_details['preceding_linker']
    assert linker['sequence'].count('G') > 0, "Linker should contain glycine residues"
    assert isinstance(linker['start'], int)
    assert isinstance(linker['end'], int)
    assert linker['start'] < linker['end']

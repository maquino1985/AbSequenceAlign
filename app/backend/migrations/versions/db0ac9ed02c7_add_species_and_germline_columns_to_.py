...
def upgrade() -> None:
    ...
    op.add_column(
        "sequence_domains", sa.Column("species", sa.String(50), nullable=True)
    )
    op.add_column(
        "sequence_domains",
        sa.Column("germline", sa.String(100), nullable=True),
    )

    # Create indexes for better query performance
    op.create_index(
        op.f("ix_sequence_domains_species"),
        "sequence_domains",
        ["species"],
        unique=False,
    )
    op.create_index(
        op.f("ix_sequence_domains_germline"),
        "sequence_domains",
        ["germline"],
        unique=False,
    )
...
def downgrade() -> None:
    # Remove indexes
    op.drop_index(
        op.f("ix_sequence_domains_germline"), table_name="sequence_domains"
    )
    op.drop_index(
        op.f("ix_sequence_domains_species"), table_name="sequence_domains"
    )

...
class JobManager:
    ...
    def create_annotation_job(self, request: MSAAnnotationRequest) -> str:
        ...
        # Start job in background thread
        thread = threading.Thread(
            target=self._process_annotation_job,
            args=(job_id, request)
        )
        thread.daemon = True
        thread.start()
    ...
    def _process_msa_job(self, job_id: str, request: MSACreationRequest):
        ...
        try:
            ...
            # Update progress
            self._update_job_status(job_id, "running", 0.3, f"Aligning {len(sequences)} sequences...")
            ...
            # Update progress
            self._update_job_status(job_id, "running", 0.7, "MSA created, preparing annotation...")
            ...
            self._update_job_status(job_id, "completed", 1.0, "MSA creation completed successfully")
            ...
            self._update_job_status(job_id, "failed", 0.0, error_msg)
    ...
    def _process_annotation_job(self, job_id: str, request: MSAAnnotationRequest):
        ...
        try:
            ...
            # Update job as completed
            self._update_job_status(job_id, "completed", 1.0, "Annotation completed successfully")

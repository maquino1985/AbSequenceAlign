"""
Value objects for the refactored biologic domain.
This includes support for DNA, RNA, and protein sequences.
"""
...
class DNASequence:
    """Value object representing a DNA sequence"""
    ...
    def _is_valid_sequence(self) -> bool:
        ...
        # Valid DNA characters (including N for unknown)
    ...
    def upper(self) -> "DNASequence":
        """Return uppercase version of the sequence"""
        return DNASequence(self.sequence.upper())

    def substring(self, start: int, end: int) -> "DNASequence":
        ...
        return DNASequence(self.sequence[start:end])
    ...
    def count_nucleotide(self, nucleotide: str) -> int:
        """Count occurrences of a specific nucleotide"""
        return self.sequence.upper().count(nucleotide.upper())

    def get_gc_content(self) -> float:
        """Calculate GC content percentage"""
        gc_count = self.count_nucleotide("G") + self.count_nucleotide("C")
    ...
    def reverse_complement(self) -> "DNASequence":
        ...
        complement = "".join(
            complement_map.get(base.upper(), base) for base in self.sequence
        )
        return DNASequence(complement[::-1])


@dataclass(frozen=True)
class RNASequence:
    """Value object representing an RNA sequence"""
    ...
    def __post_init__(self):
        if not self._is_valid_sequence():
            raise SequenceValidationError(
                f"Invalid RNA sequence: {self.sequence}",
                field="sequence",
                value=self.sequence,
            )

    def _is_valid_sequence(self) -> bool:
        """Validate that the sequence contains only valid RNA nucleotides"""
        if not self.sequence or not isinstance(self.sequence, str):
            return False

        # Valid RNA characters (including U instead of T, and N for unknown)
    ...
    def upper(self) -> "RNASequence":
        """Return uppercase version of the sequence"""
        return RNASequence(self.sequence.upper())

    def substring(self, start: int, end: int) -> "RNASequence":
        """Extract a substring from the sequence"""
        if start < 0 or end > len(self.sequence) or start > end:
            raise ValidationError(
                f"Invalid substring bounds: start={start}, end={end}, length={len(self.sequence)}",
        ...
        return RNASequence(self.sequence[start:end])
    ...
    def to_dna(self) -> DNASequence:
        ...
        dna_sequence = self.sequence.replace("U", "T").replace("u", "t")
...
@dataclass(frozen=True)
class BiologicAlias:
    """Value object representing a biologic alias"""
    ...
    def __post_init__(self):
        ...
        if not self.alias_type or not isinstance(self.alias_type, str):
            raise ValidationError(
                "Alias type must be a non-empty string",
                field="alias_type",
                value=self.alias_type,
            )
    ...
    def __eq__(self, other: object) -> bool:
        """Check equality of aliases"""
        if not isinstance(other, BiologicAlias):
            return NotImplemented
        return (
            self.alias == other.alias
            and self.alias_type == other.alias_type
            and self.source == other.source
        )

    def __hash__(self) -> int:
        """Hash for use in sets and dicts"""
        return hash((self.alias, self.alias_type, self.source))


# Type alias for all sequence types
Sequence = Union[DNASequence, RNASequence, AminoAcidSequence]

...
class SequenceRepository(Repository[AntibodySequence]):
    ...
    def __init__(self, storage_path: str = "data/sequences"):
        ...
        logger.info(f"Sequence repository initialized with storage path: {storage_path}")
    ...
    def find_by_chain_type(self, chain_type: str) -> List[AntibodySequence]:
        ...
        try:
            ...
            logger.debug(f"Found {len(matching_sequences)} sequences with chain type {chain_type}")
    ...
    def find_by_domain_type(self, domain_type: str) -> List[AntibodySequence]:
        ...
        try:
            ...
            logger.debug(f"Found {len(matching_sequences)} sequences with domain type {domain_type}")
            return matching_sequences
    ...
    def count(self) -> int:
        ...
        try:
            ...
            count = len([f for f in os.listdir(self.storage_path) if f.endswith('.json')])
    ...
    def _dict_to_entity(self, data: Dict[str, Any]) -> AntibodySequence:
        ...
        from backend.domain.entities import AntibodyChain, AntibodyDomain, AntibodyRegion
            ...
            for domain_data in chain_data.get("domains", []):
                ...
                for name, region_data in domain_data.get("regions", {}).items():
                    ...
                    region = AntibodyRegion(
                        ...
                        numbering_scheme=NumberingScheme(region_data["numbering_scheme"])
                    )
                    regions[name] = region
